// Modal for adding a new expense to a group. Supports equal and custom split types.
// Dynamically renders custom split inputs per member when custom mode is selected.
...
const AddExpenseModal = ({ group, onClose, onSuccess, currentUser }) => {
  ...
  const [customSplits, setCustomSplits] = useState(
    group?.members?.map((m) => ({ user: m._id, amount: '' })) || []
  );
  ...
  const handleCustomSplitChange = (userId, value) => {
    setCustomSplits((prev) =>
      prev.map((s) => (s.user === userId ? { ...s, amount: value } : s))
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.splitType === 'custom') {
      ...
      if (Math.abs(total - amount) > 0.01) {
        return setError(`Custom splits must total ₹${amount}. Current total: ₹${total.toFixed(2)}`);
      }
            ...
            <div className="space-y-2">
              ...
              {group?.members?.map((m) => (
                <div key={m._id} className="flex items-center gap-2">
                  <span className="text-sm text-slate-600 dark:text-slate-400 w-24 truncate" title={m.name}>{m.name}</span>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
